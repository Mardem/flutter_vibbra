...
import 'package:vibbra_notifications/modules/auth/tests/mocks/data/remote/services/google.service.fake.dart';
...
void main() {
  ...
  group('Test valid social login from repository', () {
    ...
    test('Given_UserLogin_When_TheUserWillLogin_Then_LoginIsSuccessful',
        () async {
      // Given
  ...
  group('Test invalid social login from repository', () {
    ...
    setUpAll(() {
      inject.registerFactory<GoogleAuthService>(
          () => MockGoogleAuthServiceValid());
    ...
    test(
        'Given_UserLogin_When_TheUserWillLogin_Then_LoginWillReturnExceptionBecauseProvider',
        () async {
      // Given
      // When
      SocialInfo res = await socialRepository.login();
  ...
  group('Testing Set Provider', () {
    ...
    test(
        'Given_TheRepositorySetProvider_When_ItIsCalled_Then_RepositoryReturnsNormally',
        () async {
      // Given
      // When
      // Then
      expect(
          () => socialRepository.setProvider(serviceProvider), returnsNormally);
    });
  });
}
